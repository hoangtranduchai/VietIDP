...
def detect_and_draw(img_path):
    ...
    # Use the same directory as the uploaded file
    upload_dir = os.path.dirname(img_path)
    base_name = os.path.splitext(os.path.basename(img_path))[0]

    for idx, img in enumerate(images):
        orig_filename = f"{base_name}_orig_p{idx}.jpg"
        annot_filename = f"{base_name}_annot_p{idx}.jpg"
        orig_path = os.path.join(upload_dir, orig_filename)
        out_path = os.path.join(upload_dir, annot_filename)
